...
export default function Chamada({ db, api, toast }) {
  ...
  const [presencas, setPresencas] = useState({}) // { alunoId: bool }
  ...
  function loadChamada(newSalaId, newData) {
    const sid = newSalaId ?? salaId
    const dt = newData ?? data
    const map = {}
    const alunosInSala = (db.alunos || []).filter(a => a.sala_id === sid)
    alunosInSala.forEach(a => {
      const p = (db.presencas || []).find(x => x.aluno_id === a.id && x.data === dt)
      map[a.id] = p?.presente || false
    })
    setPresencas(map)
  }

  function handleSalaChange(id) {
    setSalaId(id)
    if (data) loadChamada(id, data)
  }

  function handleDataChange(dt) {
    setData(dt)
    if (salaId) loadChamada(salaId, dt)
  }
  ...
  async function salvar() {
    ...
    try {
      ...
      const r = await api('savePresencaLote', { presencas: lote })
      const total = lote.filter(p => p.presente).length
      toast({ message: `✓ ${total} de ${alunos.length} presentes salvos! (${r.saved ?? total} gravados)` })
      ...
      <Card>
        <CardContent className="grid grid-cols-1 sm:grid-cols-3 gap-3 py-4">
          ...
          <div>
            ...
            <Select value={salaId} onChange={e => handleSalaChange(e.target.value)}>
          ...
          <div>
            ...
            <Select value={data} onChange={e => handleDataChange(e.target.value)} disabled={!salaId}>
              <option value="">— Selecione —</option>
